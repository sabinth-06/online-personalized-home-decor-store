                ...
                <div class="group relative bg-white/80 dark:bg-stone-900/80 backdrop-blur-md rounded-xl overflow-hidden whisper-shadow transition-transform duration-500 hover:-translate-y-2 flex flex-col h-full border border-stone-200/40">
                    ...
                    <div class="p-8 flex flex-col flex-1 justify-between">
                        ...
                        <div class="space-y-2 mt-auto">
                            @if ($product->in_stock)
                                <button type="button" onclick="addToCart('{{ $product->id }}', '{{ addslashes($product->name) }}', '{{ $product->base_price }}', 'Signature Collective', '{{ $productImage }}')" class="w-full py-3 rounded-lg border-[1.5px] border-stone-200 text-stone-600 font-semibold hover:bg-stone-50 hover:text-emerald-800 transition-all flex justify-center items-center gap-2 text-sm">
                                    <span class="material-symbols-outlined text-[18px]">shopping_bag</span>
                                    Add to Cart
                                </button>
                                <a href="{{ route('products.customize', $product) }}" class="w-full py-3 rounded-lg border-[1.5px] border-primary-container text-primary-container font-semibold hover:bg-primary-container hover:text-white transition-all flex justify-center items-center gap-2 text-sm">
                                    <span class="material-symbols-outlined text-[20px]">auto_fix</span>
                                    Customize
                                </a>
                            @else
                                <button type="button" disabled class="w-full py-3 rounded-lg border-[1.5px] border-stone-200 text-stone-400 bg-stone-50 dark:bg-stone-900/50 font-semibold cursor-not-allowed flex justify-center items-center gap-2 text-sm opacity-60">
